...
void HashTable::put(const string &s, int score) {
    //if string word already exists in hash table
    if (contains(s)) {
        //iterator looks to see if hashTable element matches string word passed on 
	    list<WordEntry>::iterator i;
        for (i = hashTable[computeHash(s)].begin(); i != hashTable[computeHash(s)].end(); ++i) {
            //if matches, addNewAppearance with score passed in
            if (i->getWord() == s) {
    ...
    //if string word does not already exist in hash table, call WordEntry constructor to create new element
    WordEntry entry(s, score);
    //push new element to the front of hashTable
    hashTable[computeHash(s)].push_front(entry);
}
...
double HashTable::getAverage(const string &s) {
    //if not found, default average value is 2.0 (neutral result)
    double result = 2.0;
    //if string word already exists in hash table
    if (contains(s)) {
        //iterator looks to see if hashTable element matches string word passed on 
        list<WordEntry>::iterator i;
        for (i = hashTable[computeHash(s)].begin(); i != hashTable[computeHash(s)].end(); ++i) {
            //if matches, result is the average of that element
            if (i->getWord() == s) {
                result = i->getAverage();
            }
        }
...
bool HashTable::contains(const string &s) {
    ...
    list<WordEntry>::iterator i;
    for (i = hashTable[computeHash(s)].begin(); i != hashTable[computeHash(s)].end(); ++i) {
        //if matches, return true because word exists in hashTable
        if (i->getWord() == s) {
            result = true;
        }
    }
